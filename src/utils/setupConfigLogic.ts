/**
 * setupConfigLogic.ts（renderer 可用副本）
 *
 * 提供 sanitizeSetupConfig 纯函数，用于在 Setup Wizard 的 done 子步骤
 * 调用 config:set 写入 openclaw.json 之前，过滤掉 schema 不兼容字段。
 *
 * 注意：renderer（src/）无法直接 import electron/ipc/ 的文件，
 * 因此将该纯函数复制到 src/utils/ 供 renderer 使用。
 * 逻辑与 electron/ipc/setupConfigLogic.ts 保持一致。
 *
 * 不兼容字段说明：
