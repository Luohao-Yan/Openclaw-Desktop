...
## [0.4.9-preview-4] - 2026-04-12

### ✨ 新增功能 (Features)

- **Agent 真实重命名功能**：在智能体页面新增重命名按钮，支持真实修改智能体名称（更新 `openclaw.json` 配置，非仅 UI 显示）
- **重命名对话框 UI**：完整的重命名交互流程，包含名称校验、加载状态、错误提示和成功反馈

### 🔧 优化与改进 (Improvements)

- **Agent 名称显示优化**：当智能体未设置名称时，优先显示智能体 ID 而非固定显示 "Unnamed Agent"
- **重命名 IPC 复用**：复用已有的 `agents:rename` IPC 处理器，确保重命名操作与 OpenClaw CLI 行为一致

### 📝 操作说明 (Usage)

- **重命名智能体**：在「智能体」页面，点击任意智能体卡片上的编辑图标（铅笔），在弹出的对话框中输入新名称并确认
- **命名规则**：仅允许 ASCII 字母、数字、连字符（-）和下划线（_）

---

## [0.4.9-preview-3] - 2026-04-12

### 🐛 问题修复 (Bug Fixes)

- **实例状态预加载优化**：修复实例状态预加载逻辑，补充大量 TypeScript 类型定义
- **Electron 主进程类型安全**：完善 IPC 处理器的类型标注，提升代码可维护性
...
- **版本号修正**：从 `0.4.9-preview-1` 更正为 `0.4.9-preview-2`，确保版本序列正确
...
- **侧边栏毛玻璃浮动效果**：右侧圆角 16px + `backdrop-filter: blur(20px) saturate(180%)` + 半透明背景 + 微阴影，深色/浅色主题自适应

### 🐛 修复 (Fixes)

- **IPC 超时统一修复**：所有页面 `useIpcCache` 超时从默认 5s 提升到 15-30s，修复技能/日志/任务页面频繁超时问题
